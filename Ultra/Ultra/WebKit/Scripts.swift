...
struct Script {    
    ...
    static func toJS(_ cssFile: String) -> String {
        let css = Helper.cssToString(file: cssFile, inDir: "WebCode")
        let js = "var style = document.createElement('style'); style.innerHTML = '\(css)'; document.head.appendChild(style);"
        return js
    }
    /// Grabs CSS from `cssFile`, within directory `path`, and outputs the executable JS code
    static func toJS(_ cssFile: String, path: String) -> String {
        let css = Helper.cssToString(file: cssFile, inDir: path)
        let js = "var style = document.createElement('style'); style.innerHTML = '\(css)'; document.head.appendChild(style);"
        return js
    }
}
